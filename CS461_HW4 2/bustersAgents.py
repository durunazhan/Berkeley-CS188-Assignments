...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        ghost_pos = None
        action_chosen = None
        pac_ghost_distance = float('inf')

        for dist in livingGhostPositionDistributions:
            most_likely_position = dist.argMax()
            maze_distance = self.distancer.getDistance(pacmanPosition, most_likely_position)
            if maze_distance < pac_ghost_distance:
                pac_ghost_distance = maze_distance
                ghost_pos = most_likely_position

        pac_ghost_distance = float('inf')
        for action in legal:
            successor_pos = Actions.getSuccessor(pacmanPosition, action)
            maze_distance = self.distancer.getDistance(successor_pos, ghost_pos)
            if maze_distance < pac_ghost_distance:
                pac_ghost_distance = maze_distance
