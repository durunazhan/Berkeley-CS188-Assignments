...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for pac in variableDomainsDict[PAC]:
        for ghost in variableDomainsDict[GHOST0]:
            dist = manhattanDistance(pac, ghost)
            for i in range(-MAX_NOISE, MAX_NOISE+1):
                noisy_dist = dist + i
                if noisy_dist >= 0 and abs(noisy_dist - dist) <= MAX_NOISE:
                    variableDomainsDict[OBS0].append(noisy_dist)
    variableDomainsDict[OBS0] = list(set(variableDomainsDict[OBS0]))
    variableDomainsDict[OBS0].sort()
    ...
    for pac in variableDomainsDict[PAC]:
        for ghost in variableDomainsDict[GHOST1]:
            ...
            for i in range(-MAX_NOISE, MAX_NOISE+1):
                noisy_dist = dist + i
                if noisy_dist >= 0 and abs(noisy_dist - dist) <= MAX_NOISE:
                    variableDomainsDict[OBS1].append(noisy_dist)
    variableDomainsDict[OBS1] = list(set(variableDomainsDict[OBS1]))
    variableDomainsDict[OBS1].sort()
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            newFactors, joinedFactor = joinFactorsByVariable(newFactors, var)
            joinedFactor_len = len(joinedFactor.unconditionedVariables())
            if joinedFactor_len > 1:
                newFactors.append(eliminate(joinedFactor, var))
        fullJoint = joinFactors(newFactors)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        if self.total() != 1:
            self.normalize()
        key_value_pairs = sorted(self.items())
        distribution = []
        keys = []
        for i in range(len(key_value_pairs)):
            distribution.append(key_value_pairs[i][1])
            keys.append(key_value_pairs[i][0])

        j = 0
        cumulative_prob = distribution[j]
        while rand > cumulative_prob:
            j += 1
            cumulative_prob += distribution[j]
        return keys[j]
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        ghost_in_jail = (ghostPosition == jailPosition)
        noisy_distance_is_none = noisyDistance is None

        if ghost_in_jail is True:
            if noisy_distance_is_none:
                return 1.0
            else:
                return 0.0
        if noisy_distance_is_none:
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for i in range(len(self.allPositions)):
            ...
            for new_pos in new_pos_dist.keys():
                dist[new_pos] = old_prob * new_pos_dist[new_pos] + dist[new_pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        num = int(self.numParticles / len(legal_pos))
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for part in self.particles:
            new_dist[part] += self.getObservationProb(observation, pac_pos, part, jail_pos)
        ...
        if new_dist.total() != 0:
            ...
            for _ in range(self.numParticles):
                self.particles.append(new_dist.sample())
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        new_particles = []
        for part in self.particles:
            new_part = self.getPositionDistribution(gameState, part).sample()
